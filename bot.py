...
def build_admin_embed() -> discord.Embed:
    ...
    embed.add_field(
        ...
        value=(
            ...
            "`!avisoembed ...`\n"
            "Publica avisos oficiais em embed com mídia e layout organizado."
        ),
        inline=False,
    )
...
def _parse_avisoembed_flags(raw: str) -> dict[str, str]:
    """
    Parser simples por flags:
    --titulo "..."
    --mensagem "..."
    --canal #canal (ou ID)
    --cor #RRGGBB
...
def _parse_hex_color(value: str | None) -> discord.Color:
    if not value:
...
@bot.command(name="avisoembed", hidden=True)
@commands.has_permissions(administrator=True)
async def avisoembed(ctx: commands.Context, *, args: str | None = None) -> None:
    """
    Comando administrativo para aviso oficial organizado em embed.
    Suporta anexos enviados junto com o comando e/ou URLs.
    """
    if not args:
        await ctx.send(
            "Uso:\n"
            "`!avisoembed --titulo <texto> --mensagem <texto> [--canal #canal|id] [--cor #RRGGBB] [--thumb URL] [--imagem URL] [--rodape texto]`\n"
            "Dica: você também pode anexar imagem/arquivo no próprio comando."
        )
        return

    flags = _parse_avisoembed_flags(args)
    titulo = (flags.get("titulo") or "📢 Aviso Oficial — NEXAR").strip()
    mensagem = (flags.get("mensagem") or "").strip()
    if not mensagem:
        await ctx.send("❌ Campo obrigatório ausente: `--mensagem`.")
        return

    target_channel: discord.abc.MessageableChannel = ctx.channel
    canal_raw = (flags.get("canal") or "").strip()
    if canal_raw:
        candidate = None
        if ctx.message.channel_mentions:
            candidate = ctx.message.channel_mentions[0]
        else:
            canal_id = "".join(ch for ch in canal_raw if ch.isdigit())
            if canal_id:
                candidate = ctx.guild.get_channel(int(canal_id)) if ctx.guild else None
        if candidate is None:
            await ctx.send("❌ Canal inválido em `--canal`. Use menção #canal ou ID numérico.")
            return
        target_channel = candidate

    embed = discord.Embed(title=titulo, description=mensagem, color=_parse_hex_color(flags.get("cor")))
    embed.set_author(name=f"Gerência {APP_NAME}", icon_url=bot.user.display_avatar.url if bot.user else discord.Embed.Empty)
    embed.timestamp = datetime.now(timezone.utc)
    embed.set_footer(text=(flags.get("rodape") or "Canal oficial de avisos • NEXAR").strip())

    thumb_url = (flags.get("thumb") or "").strip()
    image_url = (flags.get("imagem") or "").strip()
    ...
    try:
        ...
        await ctx.send(f"✅ Aviso enviado com sucesso em {getattr(target_channel, 'mention', '#canal')}.")
    except Exception:
        logger.exception("Erro ao enviar aviso embed")
        await ctx.send("❌ Falha ao enviar aviso embed.")
    finally:
        for f in files:
            try:
                f.close()
...
@avisoembed.error
async def avisoembed_error(ctx: commands.Context, error: commands.CommandError) -> None:
    if isinstance(error, commands.MissingPermissions):
        await ctx.send("❌ Apenas administradores podem usar `!avisoembed`.")
        return
    logger.exception("Erro em !avisoembed", exc_info=error)
    await ctx.send("Erro interno no comando de aviso embed.")
